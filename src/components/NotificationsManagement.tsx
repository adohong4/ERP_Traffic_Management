...
import { Plus, Eye, Edit, Trash2, Send, Mail, AlertTriangle, CheckCircle, Info, XCircle } from 'lucide-react';
...
interface NotificationRecord {
  ...
  targetAudience: 'all' | 'license_holders' | 'vehicle_owners' | 'violators' | 'students';
...
const mockNotifications: NotificationRecord[] = [
  {
    ...
    message: 'Kính gửi quý khách, GPLX của bạn sắp hết hạn. Vui lòng đến cơ quan CSGT để làm thủ tục gia hạn.',
    type: 'warning',
  ...
  },
  {
    ...
    message: 'Phương tiện của bạn đã đến hạn đăng kiểm. Vui lòng liên hệ trung tâm đăng kiểm gần nhất.',
    type: 'warning',
  ...
  },
  {
    ...
    message: 'Bộ GTVT vừa ban hành quy định mới về quản lý GPLX. Vui lòng xem chi tiết tại website.',
    type: 'info',
    targetAudience: 'all',
    status: 'sent',
  ...
  },
  {
    ...
    message: 'Kết quả kỳ sát hạch ngày 25/10/2024 đã được công bố. Vui lòng kiểm tra kết quả của bạn.',
    type: 'success',
  ...
  },
  {
    ...
    message: 'Bạn có vi phạm giao thông chưa được xử lý. Vui lòng đến cơ quan CSGT để giải quyết.',
    type: 'error',
...
export default function NotificationsManagement() {
  ...
  const columns: ColumnDef<NotificationRecord>[] = [
    ...
    },
    {
      ...
      width: '280px',
      render: (item) => (
        <div className="text-xs text-muted-foreground truncate max-w-[280px]" title={item.message}>
          {item.message}
        </div>
      )
    },
    ...
    },
    {
      ...
      width: '70px',
      sortable: true,
      render: (item) => <span className="text-center block text-sm">{item.recipientCount}</span>
    },
    {
      key: 'readCount',
      header: 'Đọc',
    ...
    },
    {
      ...
      sortable: true,
      render: (item) => (
        <span className="text-xs">{new Date(item.createdDate).toLocaleDateString('vi-VN')}</span>
      )
    },
    {
      key: 'actions',
          ...
          {item.status === 'draft' && (
            <TooltipProvider>
              <TooltipUI>
                ...
                <TooltipContent><p>Gửi thông báo</p></TooltipContent>
              </TooltipUI>
            </TooltipProvider>
          )}
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p>{new Date(selectedNotification.createdDate).toLocaleString('vi-VN')}</p>
              </div>
              {selectedNotification.sentDate && (
                <div>
                  <Label className="text-muted-foreground">Ngày gửi</Label>
                  <p>{new Date(selectedNotification.sentDate).toLocaleString('vi-VN')}</p>
                </div>
              )}
              <div>
            ...
            <div>
              ...
              <p className="mt-2 p-4 bg-muted rounded-lg">{selectedNotification.message}</p>
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <Card>
          <CardContent className="pt-6 space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="vehicle_owners">Chủ phương tiện</SelectItem>
            ...
            <div className="flex gap-2 justify-end pt-4">
              ...
              <Button onClick={() => setViewMode('list')} className="bg-green-600 hover:bg-green-700">
