...
export default function TrafficAuthoritiesManagement() {
  const [selectedAuthority, setSelectedAuthority] = useState<TrafficAuthority | null>(null);
  ...
  const columns: ColumnDef<TrafficAuthority>[] = [
    ...
    },
    {
      ...
      render: (auth) => (
        <div className="flex items-center gap-2 text-left w-full">
          ...
          <span className="text-sm truncate" title={auth.name}>{auth.name}</span>
        </div>
      )
    },
    {
    ...
    },
    {
      ...
      sortable: true,
      width: '120px',
      render: (auth) => <span className="text-sm truncate" title={auth.city}>{auth.city}</span>
    },
    {
      key: 'address',
      header: 'Địa chỉ',
      ...
      render: (auth) => (
        <div className="flex items-start gap-2 text-left w-full">
          ...
          <span className="text-sm text-muted-foreground truncate" title={auth.address}>{auth.address}</span>
        </div>
      )
    },
    {
      ...
      sortable: true,
      width: '150px',
      render: (auth) => <span className="text-sm truncate" title={auth.director}>{auth.director}</span>
    },
    {
      key: 'phone',
      header: 'Điện thoại',
    ...
    },
    {
      ...
      render: (auth) => (
        <div className="flex items-center gap-2 text-left w-full">
          ...
          <span className="text-xs text-muted-foreground truncate" title={auth.email}>{auth.email}</span>
        </div>
      )
    },
    {
    ...
    },
    {
      ...
      width: '130px',
      render: (auth) => (
        <Badge className={auth.status === 'active' ? 'bg-green-500 text-white' : 'bg-gray-500 text-white'}>
  ...
  const filters: FilterConfig[] = [
    {
      ...
      options: [
        ...
        ...types.map(type => ({ value: type, label: authorityTypeLabels[type] }))
      ]
    },
    {
      key: 'city',
