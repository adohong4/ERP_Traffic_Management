...
export default function LoginPage({ onLogin }: LoginPageProps) {
    ...
    const handleSignNonce = async () => {
      ...
      if (isConnected && address && !isSigning && shouldAutoSign) {
        ...
        try {
          ...
          // Success
          toast.success(
            `Xác thực thành công! Địa chỉ: ${address.slice(0, 6)}...${address.slice(-4)}`
          );
        ...
        connectors
          ...
          .filter((connector) => {
            try {
              ...
              if (typeof connector.connect !== "function") {
                console.warn(
                  "Filtering out connector without connect:",
          ...
          .map((connector) => {
            try {
              ...
              const id =
                connector.id || connector.name || `wallet-${Math.random()}`;
              let icon = "🦊";
              ...
              if (
                ...
                id.toLowerCase().includes("trust") ||
                name.toLowerCase().includes("trust")
              ) {
                icon = "🛡️";
                ...
                id.toLowerCase().includes("metamask")
              ) {
          ...
          .filter(Boolean)
      ); // Remove any null entries from mapping errors
    } catch (err) {
  ...
  const handleConnectWallet = async (wallet: (typeof displayWallets)[0]) => {
    ...
    const isWalletConnect =
      wallet.name.toLowerCase().includes("walletconnect") ||
                ...
                <div className="flex gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-blue-700/80 leading-relaxed">
                      Chúng tôi không bao giờ yêu cầu khóa riêng tư của bạn. Tất
                      cả giao dịch đều được mã hóa và bảo vệ bởi Blockchain.
                    </p>
                  </div>
                </div>
              </motion.div>
